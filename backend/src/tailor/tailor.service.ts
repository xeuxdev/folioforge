...
export class TailorService {
  ...
  async evaluateAtsCheck(
    ...
    // Build map of candidate's already accepted bullet points
    ...
    // 1. STRICTLY PRESERVE all existing accepted bullet diffs
    ...
    // Add any completely new bullet diffs generated by LLM that were not in existingDiffs
    ...
    // 2. STRICTLY PRESERVE all existing matched keywords + merge new ones
      ...
      sections: [
        {
          ...
          children: [
            ...
            new Paragraph({
              ...
              children: [
                new TextRun({
                  text: [record.targetRole, location, email]
                    .filter(Boolean)
                    .join(' | '),
                  size: 20,
                  font: 'Calibri',
  ...
  private async processTailoringWithLLM(
    ...
    try {
      ...
      throw new InternalServerErrorException(
        `AI Tailoring LLM error: ${message}`,
      );
    }
  }
}
